...
class Config:
    ...
    MAX_MEMORY: Dict[str, str] = {"cuda:0": "15GB"}  # Reserve 1GB for system
...
class KeyeVLModel:
    ...
    def load_model(self):
        ...
        try:
            ...
            # Try to load with flash attention 2
            try:
                ...
                logger.info("Loading model without Flash Attention 2...")
                self.model = AutoModel.from_pretrained(
                    self.model_path,
                    torch_dtype=config.TORCH_DTYPE,
                    device_map="auto",
...
Do not simply concatenate the segments - synthesize them into a comprehensive whole."""
    ...
    def _check_match(self, analysis: str, search_prompt: str) -> bool:
        ...
        affirmative_count = sum(1 for word in affirmative if word in first_100)
        negative_count = sum(1 for word in negative if word in first_100)
...
def main():
    ...
    try:
        logger.info("=" * 60)
        logger.info("Starting Keye-VL Video Analysis System v2.0")
        logger.info("=" * 60)
