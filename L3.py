"""
Advanced LLaVA-NeXT-Video Analysis System - Production Edition
Fully optimized with CUDA memory management and Gradio file access fixes
Optimized for RTX A4000 (16GB VRAM) + 32GB RAM
"""
...
MAX_FRAMES_PER_CLIP = 16  # Reduced for better memory management
...
RESIZE_WIDTH = 224  # Reduced for memory
RESIZE_HEIGHT = 224
...
# Temp directory for Gradio-compatible file handling
GRADIO_TEMP_DIR = tempfile.mkdtemp(prefix="llava_video_")
...
# ================================
# FILE MANAGER FOR GRADIO COMPATIBILITY
# ================================

class FileManager:
    """Manages temporary file copies for Gradio compatibility"""
    
    def __init__(self):
        self.temp_dir = GRADIO_TEMP_DIR
        self.temp_files = []
        logger.info(f"Gradio temp directory: {self.temp_dir}")
    
    def copy_to_gradio_temp(self, source_path: str) -> str:
        """
        Copy file to Gradio-accessible temp directory
        ...
        try:
            ...
            temp_filename = f"{timestamp}_{source_path.name}"
            ...
            logger.info(f"Copied {source_path.name} to temp directory")
            return temp_path
            
        except Exception as e:
            logger.error(f"Error copying file to temp: {str(e)}")
            return None
...
"
    ...
    def batch_search_videos(self, folder_path: str, search_prompt: str) -> Tuple[List[str], str]:
        """Search through videos with memory management and Gradio-compatible file handling"""
        try:
            ...
            matching_videos = []
            matching_video_temps = []  # Temp copies for Gradio
            results = []
            ...
            for video_path in video_files:
                try:
                    ...
                    if is_match:
                        ...
                        temp_path = self.file_manager.copy_to_gradio_temp(video_path)
                        if temp_path:
                            matching_video_temps.append(temp_path)
            ...
            # Return temp paths for Gradio gallery
            return matching_video_temps, markdown_report
...
class GradioInterface:
    ...
    def batch_video_interface(self, folder_path, search_prompt, progress=gr.Progress()):
        """Handle batch video search with Gradio-compatible file handling"""
        try:
            ...
            # Returns temp file paths for Gradio gallery
            matching_videos_temp, markdown_report = self.analyzer.batch_search_videos(folder_path, search_prompt)
            
            progress(1, desc="Search complete!")
            
            # Return temp paths (Gradio can access these)
            gallery_data = matching_videos_temp[:12] if matching_videos_temp else None
            
            return gallery_data, markdown_report
            
        except Exception as e:
            logger.error(f"Batch interface error: {str(e)}")
        ...
        with gr.Blocks(
            ...
            <div style="margin-top: 40px; padding: 25px; background: #f8f9fa; border-radius: 10px; border-top: 3px solid #667eea;">
                ...
                <div style="margin-top: 20px; padding: 15px; background: #e7f3ff; border-left: 4px solid #2196f3; border-radius: 5px;">
                    ...
                    Frame-level optimization | Support for long videos via auto-splitting | Gradio-compatible file handling
                </div>
            </div>
            """)
...
def main():
    ...
    print("=" * 70)
    print("🎥 Advanced Video Analysis System - Production Edition")
    print("=" * 70)
    print("
Optimizations:")
    ...
    print("  ✓ Fixed Gradio file access for batch analysis")
    print("=" * 70)
...
🌐 Launching web interface...")
        ...
        print(f"
📁 Temp directory: {GRADIO_TEMP_DIR}")
        print("
⌨️  Press Ctrl+C to stop the server")
        print("=" * 70 + "
")
        
        # Launch with temp directory cleanup on exit
        interface.launch(
            server_name="0.0.0.0",
            server_port=7860,
            share=False,
...
🛑 Shutting down gracefully...")
        ...
        try:
            if os.path.exists(GRADIO_TEMP_DIR):
                ...
                print(f"✅ Cleaned up temp directory: {GRADIO_TEMP_DIR}")
        except Exception as e:
            print(f"⚠️ Could not clean temp directory: {str(e)}")
